...
def loadImages(path, resize):
    ...
    print("loadImages _ resize: ", resize)
    print("loadImages _ image_path: ", image_path)
    list_image = []
    for _, j in enumerate(image_path):
        print("loadImages _for_ image_path : ", _, j)
        image = cv2.imread(j)
        if resize == 1:
            image = cv2.resize(
    ...
    print("loadImages _ list_image : ",len(list_image), type(list_image))
    return list_image
...
# args = {'input': './data/building', 'output': './result2', 'resize': 0}
args = {'input': r'C:\Users\yl\Desktop\sucai\cd', 'output': './result2', 'resize': 0}
print(args,type(args))  # {'input': './data/building', 'output': './result2', 'resize': 0}
...
# load images
list_images = loadImages(args["input"], args["resize"])
# list_images = [Mat1,Mat2.....]
